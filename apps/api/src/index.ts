...
app.get("/v1/catalog", async (c) => {
  ...
  cacheUrl.hostname = "catalog-cache.internal";
  ...
  if (filters.belowObserved30d) query = query.eq("below_observed_30d", true);
  ...
  const items = rows.slice(0, filters.limit).map(mapCatalogItem);
...
app.get("/v1/catalog/facets", async (c) => {
  const { data, error } = await c.get("db").rpc("catalog_facets");
...
app.get("/v1/products/:id", async (c) => {
  ...
  return c.json({ ...mapCatalogItem(product), history: history ?? [] });
});
...
function mapCatalogItem(row: Record<string, any>) {
  return { id: row.id, externalId: row.external_id, name: row.name, brand: row.brand, productUrl: row.product_url,
    imageUrls: row.image_urls ?? [], colorOriginal: row.color_original, colorFamily: row.color_family, categories: row.categories ?? [],
