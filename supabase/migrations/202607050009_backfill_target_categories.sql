-- Every active product collected by a category target belongs to that target's
-- category. This repairs products saved before the sync worker began retaining
-- the target label alongside source-provided breadcrumb leaves.
insert into public.categories(slug, name)
select 'category-' || md5(lower(target.label)), target.label
from (
  select distinct on (lower(t.label)) t.label
  from public.sync_targets t
  where t.kind = 'category'
  order by lower(t.label), t.label
) target
where not exists (
    select 1 from public.categories c where lower(c.name) = lower(target.label)
  )
...
-- Preserve every active category-target membership after an exact source
-- breadcrumb replaces stale heuristic categories. A product may be shared by a
-- category target and a brand/search target, regardless of sync order.
...
begin
  ...
  loop
    ...
    for v_category in
      select jsonb_array_elements_text(coalesce(item->'categories', '[]'::jsonb))
      union
      select t.label
      from public.sync_target_products stp
