create or replace function public.catalog_excluded_basics_categories() returns text[]
...
language sql stable security definer set search_path = public as $$
  with filter_values as (
    ...
    select c.*,
      ...
      (not premium_ok)::integer + (not basics_ok)::integer + (not price_ok)::integer + (not common_ok)::integer as failed_groups
    from checks c
  )
...
grant execute on function public.catalog_excluded_basics_categories() to service_role;
