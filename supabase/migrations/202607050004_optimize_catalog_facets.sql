-- Parse every filter once and scan the aggregated catalog view once. Each row
-- stores the number of failed facet groups, so a contextual facet only needs
-- to subtract its own failure instead of evaluating the full matcher again.
